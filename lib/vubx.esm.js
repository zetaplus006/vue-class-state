...
var ClassInjector = /** @class */ (function (_super) {
...
var ValueInjector = /** @class */ (function (_super) {
...
var FactoryInjector = /** @class */ (function (_super) {
...
function mutation(target, methodName, descriptor) {
...
var unnamedName = '<unnamed mutation>';
function runInMutaion(ctx, func, payload, mutationType) {
    var rootScope = ctx.__scope__.$root.__scope__, scope = ctx.__scope__, meta = DIMetaData.get(ctx), mType = mutationType || unnamedName;
    var vubxMutation = {
    ...
    var result = func.apply(ctx, payload);
...
var Service = /** @class */ (function () {
    ...
    Service.prototype.mutation = function (fn, mutationType) {
...
var UseMap = typeof Map === 'function' ? Map : SimpleMap;

var Provider = /** @class */ (function () {
    function Provider() {
        this.injectorMap = new UseMap();
        /**
    ...
    Provider.prototype.get = function (identifier) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            assert(injector, String(identifier) + " not find in provider");
        }
    ...
    Provider.prototype.checkIdentifier = function (identifier) {
        if (process.env.NODE_ENV !== 'production') {
            assert(!this.injectorMap.has(identifier), "The identifier " + String(identifier) + " has been repeated");
        }
    };
    /**
...
function definedComputed(proto, getterKeys) {
    var $getters = {};
    getterKeys.forEach(function (key) {
        def(proto, key, {
            get: function () {
...
function getPropertyGetters(target) {
    var getters = {};
    var injectMeta = ClassMetaData.get(target).injectMeta;
    var keys = Object.getOwnPropertyNames(target);
    keys.forEach(function (key) {
        // skip @lazyInject
        if (key === 'constructor' || injectMeta.has(key)) {
            return;
        }
...
function createDecorator(vueConstructor) {
...
function createVubxClass(vueConstructor, constructor, decoratorOption) {
    var classMeta = ClassMetaData.setGetterMeta(constructor.prototype);
    ...
    return /** @class */ (function (_super) {
        ...
        function Vubx() {
            ...
            var meta = ClassMetaData.get(constructor.prototype);
            var vm = new vueConstructor({
                data: _this,
                computed: bindGetters(meta.getterMeta, meta.getterKeys, _this)
            });
            ...
            scope.$vm = vm;
            proxyState(_this, meta.getterKeys);
            proxyGetters(_this, vm, meta.getterKeys);
            if (decoratorOption && decoratorOption.root) {
                ...
                provider_1.registerInjectedHook(function (instance, diMetaData) {
                    if (instance instanceof Service) {
                ...
                var diMeta = DIMetaData.get(_this);
                diMeta.identifier = option.identifier;
                diMeta.provider = provider_1;
                createdHook(_this, option, diMeta);
...
function bindGetters(getters, keys, ctx) {
    var returnGetters = {};
    keys.forEach(function (key) {
        returnGetters[key] = {
...
function lazyInject(option, propertyKey) {
...
function createLazyDecorator(target, propertyKey, identifier) {
...
export { Service, bind, createDecorator, mutation, lazyInject, created };
