...
function lazyInject(identifier) {
    return function (targe, propertyKey, desc) {
        var instance;
        var getter = function () {
            if (instance) {
                return instance;
            }
            var provider = this.$service.getProvider();
            instance = provider.getInstance(identifier);
            appendServiceChild(this.$service, propertyKey, instance, identifier);
            return instance;
        };
        if (desc) {
            // for es6 decorator
...
var Service = (function () {
    ...
    Service.prototype.useStrict = function (isStrict) {
        var _this = this;
        if (isStrict === void 0) { isStrict = true; }
        if (isStrict && process.env.NODE_ENV !== 'production') {
            this.__.$vm && this.__.$vm.$watch(function () {
                return this.$data;
            }, function (val) {
                assert(_this.__.isCommitting, 'Do not mutate vubx service data outside mutation handlers.');
            }, { deep: true, sync: true });
        }
        return this;
    };
    Service.prototype.useDevtool = function (useDevtool) {
        if (useDevtool === void 0) { useDevtool = true; }
        ...
        return function (constructor) {
            return (function (_super) {
                ...
                function Vubx() {
                    ...
                    vm.$service = _this;
                    if (option) {
                        var root = option.root, identifier = option.identifier, _a = option.provider, provider = _a === void 0 ? [] : _a, _b = option.injector, injector = _b === void 0 ? [] : _b, _c = option.plugins, plugins = _c === void 0 ? [] : _c;
                        if (root) {
                            __.$root = _this;
...
function proxyGetters(ctx, vm, getterKeys) {
    ...
    getterKeys.forEach(function (key) {
        def(ctx, key, __assign({ get: function () { return vm[key]; }, set: function (value) { return vm[key] = value; } }, defaultConfig));
...
function appendServiceChild(parent, childName, child, identifier) {
    ...
    def(parent.__.$state, childName, __assign({ get: function () { return child.__.$state; } }, defaultConfig));
    def(parent.__.$getters, childName, __assign({ get: function () { return child.__.$getters; } }, defaultConfig));
}
...
export { Service, bindClass, bindFactory, createDecorator, mutation, lazyInject };
